...
class COCO_CUSTOM(Base):
    ...
    CATEGORY_TO_LABEL_DICT = {
        ...
        'bench': 15, 'bird': 16, 'cat': 17, 'dog': 18, 
        'sheep': 20, 
       'backpack': 27, 'umbrella': 28, 'handbag': 31, 'tie': 32, 'suitcase': 33, 'frisbee': 34,
         'sports ball': 37,  'baseball bat': 39,
        'baseball glove': 40, 'skateboard': 41,  'tennis racket': 43, 'bottle': 44,
        'wine glass': 46, 'cup': 47, 'fork': 48, 'knife': 49,
        'spoon': 50, 'bowl': 51, 'banana': 52, 'apple': 53, 'sandwich': 54,
        'orange': 55, 'broccoli': 56, 'pizza': 59,
        'donut': 60, 'cake': 61, 'chair': 62, 'couch': 63, 'potted plant': 64,
        'bed': 65, 'dining table': 67, 
        'toilet': 70,  'tv': 72, 'laptop': 73, 'mouse': 74,
        'remote': 75, 'keyboard': 76, 'cell phone': 77, 'microwave': 78, 'oven': 79,
        'toaster': 80, 'sink': 81, 'refrigerator': 82,  'book': 84,
    ...
    def __init__(self, path_to_data_dir: str, mode: Base.Mode, image_min_side: float, image_max_side: float):
        ...
        path_to_coco_dir = os.path.join(self._path_to_data_dir, 'coco_custom')
        path_to_annotations_dir = os.path.join(path_to_coco_dir, 'annotations')
        path_to_caches_dir = os.path.join('caches', 'coco_custom', f'{self._mode.value}')
        ...
        if self._mode == COCO_CUSTOM.Mode.TRAIN:
            path_to_jpeg_images_dir = os.path.join(path_to_coco_dir, 'train')
            path_to_annotation = os.path.join(path_to_annotations_dir, 'instances.json')
        elif self._mode == COCO_CUSTOM.Mode.EVAL:
        ...
        if os.path.exists(path_to_image_ids_pickle) and os.path.exists(path_to_image_id_dict_pickle):
            ...
            for idx, (image, annotation) in enumerate(tqdm(coco_dataset)):
                if len(annotation) > 0:
                    ...
                    self._image_id_to_annotation_dict[image_id] = COCO_CUSTOM.Annotation(
                        filename=os.path.join(path_to_jpeg_images_dir, '{:012d}.jpg'.format(int(image_id))),
                        objects=[COCO_CUSTOM.Annotation.Object(
    ...
    def __getitem__(self, index: int) -> Tuple[str, Tensor, Tensor, Tensor, Tensor]:
        ...
        if self._mode == COCO_CUSTOM.Mode.TRAIN and random.random() > 0.5:
            image = ImageOps.mirror(image)
            bboxes[:, [0, 2]] = image.width - bboxes[:, [2, 0]]  # index 0 and 2 represent `left` and `right` respectively

        image, scale = COCO_CUSTOM.preprocess(image, self._image_min_side, self._image_max_side)
